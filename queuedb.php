...
function updateEntry() {
    try {
        ...
        $conn = new \PDO('sqlite:/local/my_web_files/selarkin/cclc/cclc.db',
        ...
        $stat = $conn->prepare('UPDATE Question 
                SET time_answered = datetime(\'now\', \'localtime\'), 
                    removed_by=:remover 
                WHERE
                    num = :nums');
        echo "prepared...\n";
...
function getEntries() {
    try {
        $loc = 'sqlite:/local/my_web_files/selarkin/cclc/cclc.db';
        ...
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $vals = [];
        $stat = $conn->query('SELECT * FROM Question WHERE removed_by = 0');
...
function addEntry() {
    try {
        ...
        echo "attribute set\n";
        # the line below uses prepare and :name to prevent SQL injection
        $stat = $conn->prepare('INSERT INTO Question VALUES(null, :uid, :class,
:quest, datetime(\'now\', \'localtime\'), null, 0)');
        echo "prepared...\n";
        $uid = $_POST["uuid"];
        $class = $_POST["class"];
        $quest = $_POST["quest"];
        $res = $stat->execute([':uid'=>$uid, ':class'=>$class, ':quest'=>$quest]);
        echo "exec'd\n";
#echo "yippee!";
        echo $res;
